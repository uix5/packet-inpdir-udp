...
do
    ...
    function p_inputdir.dissector (buf, pinfo, tree)
    	if tree then
    		...
    		-- not even magic can fit in this
    		if pkt_len < (4 * 3) then 
    			subtree:add_expert_info(PI_MALFORMED, PI_ERROR, "Packet too small")
    			return
    		end
    		...
    		-- 
    		local pkt_type = buf(20, 4):le_uint()
    		...
    		-- see if there is one, if not, use generic decoder
    		local offset = 0
